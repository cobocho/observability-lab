...
### 실습: Remote Write로 VictoriaMetrics 연동

[7장 실습](7.Prometheus-아키텍처.md) 환경에 VictoriaMetrics를 추가한다.

**1) docker-compose.yml에 VictoriaMetrics 추가**
...
**2) prometheus.yml에 Remote Write 추가**
...
**4) VictoriaMetrics에서 쿼리 확인**

브라우저에서 `http://localhost:8428/vmui/` 접속 후 PromQL 쿼리 실행:
...
Prometheus와 동일한 결과가 나오는지 확인한다.

**5) Grafana에 VictoriaMetrics Data Source 추가**
...
이제 Prometheus(단기 데이터)와 VictoriaMetrics(장기 데이터) 두 곳에서 동일한 데이터를 쿼리할 수 있다.

**6) Remote Write 상태 확인**
