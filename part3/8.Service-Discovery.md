...
7장에서 만든 환경에서 `static_configs`를 File SD로 전환해본다.

먼저 `prometheus/` 디렉토리 안에 `targets/` 디렉토리를 만들고 타겟 파일을 작성한다.
...
`node-exporter` job의 `static_configs`를 `file_sd_configs`로 교체한다:
...
Prometheus 컨테이너가 타겟 파일을 읽을 수 있도록 볼륨을 추가한다:
...
`docker compose restart prometheus`로 재시작한 뒤, Prometheus UI → Status → Targets에서 `node-exporter` job의 Discovered Labels에 `env="lab"`, `type="infra"`가 추가된 것을 확인한다.

**동적 변경 확인**

File SD의 진짜 가치는 여기서부터다. 타겟 파일에 Prometheus 자체를 추가해본다:
...
파일을 저장하고 Prometheus를 재시작하지 않는다. `refresh_interval: 10s`로 설정했으므로, 10초 이내에 Targets 페이지에 새 타겟이 자동으로 나타난다. `prometheus.yml`을 수정하지 않고도 타겟을 추가/제거할 수 있다는 것이 File SD의 핵심이다.
