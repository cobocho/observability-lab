...
## 9.4 해보기: Relabeling으로 메트릭 필터링

7장에서 만든 환경을 사용한다. Node Exporter는 수백 개의 메트릭을 노출하는데, 실제로 필요한 것만 남겨보자.

먼저 현재 시계열 수를 확인한다. Prometheus UI에서 다음 쿼리를 실행한다:

```promql
count({job="node-exporter"})
```

결과를 기억해두자. 보통 500~1000개가 나온다.

이제 `prometheus.yml`의 `node-exporter` job에 `metric_relabel_configs`를 추가한다:
...
1분 정도 기다린 후 다시 `count({job="node-exporter"})`를 실행해보자. 시계열 수가 크게 줄어든 것을 확인할 수 있다. `metric_relabel_configs`의 `keep` 액션이 지정한 패턴에 매칭되지 않는 모든 메트릭을 버린 것이다.

### Label 변환 실험

이번에는 Target Relabeling으로 Label을 조작해본다. `prometheus.yml`의 `node-exporter` job에 `relabel_configs`를 추가한다:
...
`instance` Label도 정리해보자. 기본적으로 `instance`는 `host:port` 형태인데, 포트를 제거하고 호스트 이름만 남길 수 있다:
